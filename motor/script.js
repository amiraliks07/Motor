...
function playMotorSound() {
  const sound = document.getElementById("motorSound");
  sound.currentTime = 0; 
  sound.play();
}


function triggerMotorRide() {
  const motor = document.createElement('div');
  motor.className = 'motor-slide';
  motor.innerHTML = '<img src="styel/imgs/Copilot_20250926_065133.png" alt="motor" />';
...
function showSmoke() {
  const container = document.getElementById('smokeBox');

  for (let i = 0; i < 2; i++) {
    const smoke = document.createElement('div');
    smoke.className = 'smoke';
    smoke.style.left = `${i * 60}px`; 
